...
export default function ElencoForm() {
    ...
    const InputBox = ({ key, label }: { key: keyof typeof form, label: string }) =>
        <FloatLabel key={key} className="w-full">
    ...
    return <div>
        <DataTable selectionMode={"single"} selection={selected} dataKey={"Id"} onRowUnselect={e => setSelected(null)} onSelectionChange={e => setSelected(e.value as ElencoModel)} value={data?.elencos} paginator rows={5} rowsPerPageOptions={[5, 10, 25, 50]} tableStyle={{ minWidth: '50rem' }}>
